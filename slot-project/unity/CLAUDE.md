...
## Neon Fortune specifics

- The game logic (math, stops, wins, FS state) lives in the SDK package (`../sdk/package`, `SlotSdk` asmdef). **Put no math logic in Unity.**
- `Scripts/Runtime/`:
  - `SlotController` … game flow (game spec "Game Flow" sheet). A coroutine state machine
  - `SlotUi` / `Ui` … builds the whole screen in code (no prefabs or scene editing needed)
  - `ReelView` + `ReelMotion` … reel animation. `ReelMotion` is pure logic (covered by EditMode tests)
  - `GameSpec` … constants from the game spec (keep in sync with `../tools/build_game_spec.py`)
  - `ProceduralSprites` / `SynthAudio` … art and sound effects generated in code
- `Resources/Math/neon_fortune.json` is a copy. The source of truth is `../math/neon_fortune.json` (synced by `../tools/make_specs.sh`; `MathResourceTests` catches drift)
- The scene `Assets/_Game/Scenes/Main.unity` is created automatically by `Scripts/Editor/SceneSetup.cs` (don't hand-edit the YAML)
- Menu: `Neon Fortune > Rebuild Main Scene` / `Reset Save Data`
